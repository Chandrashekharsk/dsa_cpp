...
int maxProfit3(vector<int>& prices, int cap) {
  // here cap vec is size 3 because it can be 0|1|2
  vector<vector<int>> next(2, vector<int>(cap+1, 0)), curr(2, vector<int>(cap+1, 0));
  for (int buy = 0; buy <= 1; ++buy) {
    for (int c = 0; c <= cap; ++c) {
      next[buy][c] = curr[buy][c] = 0;
    }
  }
  for (int buy = 0; buy <= 1; ++buy) {
...
int main() {
  ...
  cout << maxProfit2(prices, 3)<<endl;
  cout << maxProfit3(prices, 3)<<endl;
  cout << maxProfit4(prices, 3)<<endl;
